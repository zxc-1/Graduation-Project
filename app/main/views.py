...
def find_record():
    records = db.session.query(ReadBook).join(Inventory, ReadBook.barcode == Inventory.barcode).join(Book, Inventory.isbn == Book.isbn).filter(ReadBook.card_id == request.form.get('card'))\
        .with_entities(ReadBook.barcode, Inventory.isbn, Book.book_name, Book.author, ReadBook.start_date,
                       ReadBook.end_date, ReadBook.due_date).all()  # with_entities
    data = []
...
def find_not_return_book():
    ...
    if stu.loss is True:
        return jsonify([{'stu': 3}])  # 已经挂失
    books = db.session.query(ReadBook).join(Inventory, ReadBook.barcode == Inventory.barcode).join(Book, Inventory.isbn == Book.isbn).filter(ReadBook.card_id == request.form.get('card'),
...
def bookin():
    ...
    db.session.add(book)
    db.session.commit()
    bks = db.session.query(ReadBook).join(Inventory, ReadBook.barcode == Inventory.barcode).join(Book, Inventory.isbn == Book.isbn).filter(ReadBook.card_id == card,
        ReadBook.end_date.is_(None)).with_entities(ReadBook.barcode, Book.isbn, Book.book_name, ReadBook.start_date,
                                                 ReadBook.due_date).all()
    data = []
    for bk in bks:
